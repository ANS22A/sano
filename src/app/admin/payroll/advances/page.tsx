    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Manage employee cash advances.</p>
        </div>
          ...
          <table className="w-full text-sm text-left rtl:text-right text-muted-foreground">
            ...
            <tbody className="divide-y divide-neutral-100">
              {advances.map(advance => (
                <tr key={advance.id} className="hover:bg-muted/50 transition-colors">
                  <td className="px-6 py-4 font-medium text-foreground">{advance.staff?.name_en || 'Unknown'}</td>
