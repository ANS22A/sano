    ...
    <div className="max-w-2xl space-y-6">
      ...
      >
        ...
        <div className="flex items-center justify-end gap-3 pt-4 border-t border-border-subtle">
          <Link
            href={bookingId ? `/admin/bookings/${bookingId}` : '/admin/sales'}
            className="px-3 py-2 sm:px-4 sm:py-2.5 rounded-xl border border-border-subtle text-xs font-semibold text-muted-foreground dark:text-muted-foreground hover:bg-surface dark:hover:bg-primary transition-colors"
          >
            {isAr ? 'إلغاء' : 'Cancel'}
          </Link>
          <button
            type="submit"
            disabled={isPending}
            className="inline-flex items-center gap-1.5 sm:gap-2 px-5 py-2.5 rounded-xl bg-primary text-white text-xs font-semibold hover:bg-primary-hover transition-colors shadow-sm disabled:opacity-50"
          >
