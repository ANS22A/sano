  ...
  return (
    <div className="bg-white rounded-2xl border border-[#e8ddd0] overflow-hidden">
      <div className="px-6 py-4 border-b border-[#e8ddd0] flex items-center gap-3 bg-[#faf7f4]">
        <CalendarX className="w-5 h-5 text-[#c9a96e]" />
        <span className="text-sm font-medium text-[#2a2118]">Blackout Dates</span>
      </div>
        ...
        <form onSubmit={handleAdd} className="flex flex-col sm:flex-row gap-3 items-end mb-6">
          <div className="flex-1 min-w-0">
            ...
            <input
              ...
              name="date"
              required
              className="w-full px-4 py-2.5 rounded-xl border border-[#e8ddd0] bg-white text-sm focus:outline-none focus:ring-2 focus:ring-[#c9a96e]"
            />
          </div>
          ...
          <button
            type="submit"
            disabled={isPending}
            className="px-6 py-2.5 rounded-xl bg-[#2a2118] text-white text-sm font-medium hover:bg-[#3a3128] disabled:opacity-60 transition-colors shrink-0"
          >
            Add Date
          </button>
        </form>

        <div className="divide-y divide-[#f0e8de] border-t border-[#f0e8de]">
          {activeDates.length === 0 ? (
            ...
            activeDates.map((d) => (
              <div key={d.id} className="flex items-center justify-between py-4">
                <div>
                  <p className="text-sm font-medium text-[#2a2118]">{new Date(d.date).toLocaleDateString()}</p>
