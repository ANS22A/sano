...
export default async function AccountBookingDetailPage(props: PageProps) {
  ...
  return (
    <div className="space-y-8 max-w-2xl">
      <Link href={`/${locale}/account/bookings`} className="inline-flex items-center gap-2 text-sm font-medium text-[#A98FB8] hover:text-[#6F4E7C] transition-colors">
      ...
      <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-[#E7DBEC]">
        ...
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4 mb-8 pb-8 border-b border-[#E7DBEC]">
          <div>
            <h1 className="text-2xl font-bold text-[#2E1F38] font-serif mb-2">{serviceName}</h1>
            <p className="text-[#6F4E7C] font-medium text-sm tracking-wide uppercase">Booking #{booking.id.slice(0, 8)}</p>
          </div>
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-start gap-4">
            <div className="w-10 h-10 rounded-full bg-[#faf7f4] flex items-center justify-center shrink-0">
              <Clock className="w-5 h-5 text-[#6F4E7C]" />
            </div>
            <div>
          ...
          <div className="flex items-start gap-4">
            <div className="w-10 h-10 rounded-full bg-[#faf7f4] flex items-center justify-center shrink-0">
              <CreditCard className="w-5 h-5 text-[#6F4E7C]" />
            </div>
            <div>
          ...
          <div className="flex items-start gap-4">
            <div className="w-10 h-10 rounded-full bg-[#faf7f4] flex items-center justify-center shrink-0">
              <MapPin className="w-5 h-5 text-[#6F4E7C]" />
            </div>
            <div>
              <p className="text-sm font-bold text-[#A98FB8] uppercase tracking-wider mb-1">Location</p>
              <p className="font-medium text-[#2E1F38]">{locationName}</p>
              {address && <p className="text-sm text-[#6F4E7C] mt-1">{address}</p>}
            </div>
          </div>
