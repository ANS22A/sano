...
export function AdminEmptyState({ icon, title, description, action }: AdminEmptyStateProps) {
  return (
    <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
      {icon && (
        <div className="w-16 h-16 rounded-[20px] bg-gradient-to-b from-[#faf7f4] to-[#f5ede0] flex items-center justify-center mb-5 text-accent shadow-sm border border-border">
          {icon}
        </div>
      )}
