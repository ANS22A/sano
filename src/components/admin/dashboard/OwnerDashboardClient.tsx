    ...
    <div className="space-y-8">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard
            ...
            value={`${Number(stats.realized_revenue).toLocaleString()} SAR`}
            icon={<DollarSign className="w-5 h-5" />}
            accent
          />
          <StatCard
            title={t.ownerDashboard.operatingExpenses}
            value={`${Number(stats.operating_expenses).toLocaleString()} SAR`}
            icon={<ArrowDownRight className="w-5 h-5" />}
          />
          <StatCard
            title={t.ownerDashboard.purchases}
            value={`${Number(stats.purchases).toLocaleString()} SAR`}
            icon={<ShoppingCart className="w-5 h-5" />}
          />
          <StatCard
            title={t.ownerDashboard.salariesPaid}
            value={`${Number(stats.salaries_paid).toLocaleString()} SAR`}
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
          <StatCard
            ...
            value={`${Number(stats.net_operating_profit).toLocaleString()} SAR`}
            icon={<Activity className="w-5 h-5" />}
            accent
          />
          <StatCard
            title={t.ownerDashboard.partnerWithdrawals}
            value={`${Number(stats.partner_withdrawals).toLocaleString()} SAR`}
            icon={<Wallet className="w-5 h-5" />}
          />
          <StatCard
            title={t.ownerDashboard.netCashMovement}
            value={`${Number(stats.net_cash_movement).toLocaleString()} SAR`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <StatCard
          ...
          value={`${Number(stats.expected_revenue).toLocaleString()} SAR`}
          subtitle="From Bookings"
          icon={<DollarSign className="w-5 h-5 text-gray-400" />}
        />
        <StatCard
          title={t.ownerDashboard.outstandingBalance}
          value={`${Number(stats.outstanding_balance).toLocaleString()} SAR`}
