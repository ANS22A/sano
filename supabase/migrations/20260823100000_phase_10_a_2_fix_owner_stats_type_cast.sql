-- Fix for get_owner_financial_stats
-- Removes invalid ::text casts causing 42883 on date columns

CREATE OR REPLACE FUNCTION public.get_owner_financial_stats(p_start_date date, p_end_date date)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  FROM expenses
  WHERE is_archived = false AND date >= p_start_date AND date <= p_end_date;
  ...
  WHERE b.status = 'completed' AND b.date >= p_start_date AND b.date <= p_end_date;
