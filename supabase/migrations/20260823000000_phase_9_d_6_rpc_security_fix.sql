-- ==============================================================================
-- PHASE 9-D.6: OWNER DASHBOARD RPC SECURITY FIX
-- ==============================================================================
-- This migration updates the get_owner_financial_stats RPC to explicitly
-- handle NULL values for auth.uid() and v_user_role, preventing an issue where
-- NULL != 'admin' evaluates to FALSE instead of TRUE in PL/pgSQL.
...
DECLARE
  ...
  v_user_role text;
BEGIN
  -- 1. Security Check (Fixed NULL handling)
